...
function Badge({ chambre }: { chambre: string }) {
  const c = CHAMBRE[chambre] || { label: chambre, short: chambre, color: '#7a90a8', bg: 'rgba(122,144,168,.15)' }
...
function Photo({ elu }: { elu: Elu }) {
  ...
  const url = !err && (elu.photo_url || (elu.chambre==='assemblee'&&elu.an_id?`${base}/assemblee/${elu.an_id}.jpg`:elu.chambre==='senat'&&elu.matricule?`${base}/senat/${elu.matricule}.jpg`:elu.chambre==='europarl'&&elu.ep_id?`${base}/europarl/${elu.ep_id}.jpg`:null))
  return (
    <div style={{ width:'100%', height:'100%', background:'#1a2333', display:'flex', alignItems:'center', justifyContent:'center', overflow:'hidden', borderRadius:'inherit' }}>
      {url ? <img src={url} alt="" style={{ width:'100%', height:'100%', objectFit:'cover' }} onError={() => setErr(true)} /> : <span style={{ color:'#7a90a8', fontSize:'1rem', fontWeight:600 }}>{elu.prenom?.[0]}{elu.nom?.[0]}</span>}
    </div>
  )
}
...
export default function ElusPage() {
  const [q, setQ] = useState(''); const [chambre, setChambre] = useState(''); const [elus, setElus] = useState<Elu[]>([]); const [total, setTotal] = useState(0); const [page, setPage] = useState(1); const [loading, setLoading] = useState(false); const [selected, setSelected] = useState<Elu|null>(null)
              ...
              <button key={e.id} onClick={() => setSelected(selected?.id===e.id?null:e)}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-muted text-xs truncate mt-0.5">
                    {e.groupe&&`${e.groupe} · `}
                    {e.chambre==='assemblee'?(e.departement||'Assemblée nationale'):e.chambre==='senat'?'Sénat':'Parlement européen'}
                  </p>
                </div>
