...
export async function POST(req: NextRequest) {
  try {
    // Point 7 : Validation Zod stricte
    ...
    if ((count ?? 0) > 0) {
      return NextResponse.json(
        { error: 'Témoignage déjà soumis pour cet élu.' },
        { status: 409 }
      )
    }
    ...
    try { await db.from('audit_logs').insert({
      ...
      new_data:    { elu_id: b.eluId, elu_chambre: b.eluChambre, score_global: scoreGlobal },
      ip_address:  req.headers.get('x-forwarded-for') || req.headers.get('x-real-ip'),
    }) } catch {}

    const resend = new Resend(process.env.RESEND_API_KEY)

    // Email confirmation collab
      ...
      html: `
        <div style="font-family:sans-serif;max-width:500px;margin:40px auto;background:#161e2d;border:1px solid #1f2d42;border-radius:12px;overflow:hidden">
          <div style="background:#e8b84b;padding:20px 28px">
            <h2 style="margin:0;color:#0d1117">🛡️ VigieParl — Nouveau témoignage</h2>
          </div>
