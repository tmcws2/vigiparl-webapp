...
export async function GET() {
  try {
    ...
    let noteMoyenne = null, totalContributions = 0
    ...
    if (scores && scores.length > 0) {
      const globals = scores.map((s: Record<string,number>) => s.score_global).filter(Boolean)
      noteMoyenne = globals.length > 0 ? Math.round(globals.reduce((a:number,b:number) => a+b,0)/globals.length*10)/10 : null
      totalContributions = scores.reduce((a:number,s:Record<string,number>) => a+(s.contributions_count||0),0)
      ...
      Object.entries(keys).forEach(([k, col]) => {
        ...
        moyennesParCritere[k] = vals.length > 0 ? Math.round(vals.reduce((a:number,b:number) => a+b,0)/vals.length*10)/10 : null
      })
    }
    const durees = mandats?.map((m: Record<string,number>) => m.duree_jours).filter((d: number) => d && d > 0) || []
    ...
    return NextResponse.json({ vigiparl: { noteMoyenne, totalContributions, elusAvecScores: scores?.length||0, moyennesParCritere }, global: { elusActifs: elusCount, collabsActifs: collabsCount, ancienneteMoyenneMois, tailleMoyenne, turnoverMoyen } })
